...
class ReadManifest(
    ...
    fun parse(): Data {
        val aXml = AXmlDecoder.decode(FileInputStream(manifest))
        return Data(AXmlResourceParser().apply {
            open(ByteArrayInputStream(aXml.data), aXml.mTableStrings)
        })
    }

    class Data(
        private val parser: AXmlResourceParser
    ) {
        companion object {
            /**
            ...
            private const val APP_COMPONENT_NAME = 0x0101057a
        ...
        val appComponentFactoryName by lazy(LazyThreadSafetyMode.NONE) {
            findAttributeStringValue("application", APP_COMPONENT_NAME)
        }
